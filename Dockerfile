...
ENV PATH /opt/conda/envs/build3/bin:/opt/conda/bin:$PATH
...
COPY conda/build3.lock.yml /environment.yml
...
RUN conda init bash \
    ...
    && echo '. /opt/conda/bin/activate build3' >> ~/.bashrc \
    && . ~/.bashrc \
...
RUN echo '# Always activate conda' >> ${AIRFLOW_USER_HOME}/.bashrc \
    && echo '. /opt/conda/bin/activate build3' >> ${AIRFLOW_USER_HOME}/.bashrc
